...
- 생성자 : 객체가 생성될 때 자동으로 실행. 멤버 변수 초기화, 클래스이름() 형태
    ...
    - `변환 생성자` : 인자를 한 개만 가지는 생성자. 다른 타입에서 해당 클래스 타입으로 자동 형변환 가능. explicit 키워드 사용 시 자동 형변환 방지

    - `복사 생성자` : 다른 객체를 이용해서 새로운 객체 생성. const & 연산자 사용
        - 기본 복사는 얕은 복사이기 때문에 포인터 멤버가 있다면 깊은 복사 생성자 직접 구현 필요
        - 스택에 생성할 때는 문제가 안 되지만 힙에서 생성된 객체가 소멸될 때 메모리 충돌 위험이 있음
...
### 다형성
#### 정적 다형성
- 다형성 : 같은 이름의 함수가 상황에 따라 다르게 동작하는 것
...
### 얕은복사와 깊은복사
#### 얕은 복사
- 주소 값만 복사. 원본과 복사본이 같은 메모리 공간(객체)을 참조함.
- 하나 수정하면 둘 다 영향
...
#### 깊은 복사
- 객체의 내용을 새로운 메모리 공간에 완전히 복사.
- 원본과 복사본이 독립적으로 존재하기 때문에 수정해도 서로 영향 X
